...
export default function WineInventoryDetailsPage() {
  ...
  const [wineData, setWineData] = useState<IWineInventory | undefined>(undefined);
  ...
  useEffect(() => {
    async function loadWine() {
      ...
      try {
        ...
        // Find wine from mock data
        const mockWine = _wineInventoryList.find((w) => w.id === id);
        if (mockWine) {
          setWineData(mockWine);
        } else {
        ...
        setLoading(false);
        enqueueSnackbar('Failed to load wine', { variant: 'error' });
      }
    }
  ...
  const handleFieldSave = useCallback(
    async (fieldName: string, value: string | number | string[]) => {
      ...
      try {
        ...
        console.log('✅ [WINE FIELD SAVED]', { wineId: wineData.id, field: fieldName, value });
                  ...
                  >
                    <Stack spacing={1}>
                      ...
                      <Stack direction="row" spacing={1} alignItems="center">
                        ...
                        <Chip label={wineData.wineType} size="small" color="primary" />
                      </Stack>
                    </Stack>
                  </Card>
                </Grid>
