...
const buildEditor = ({
  ...
  const getWorkspace = () => {
    return canvas.getObjects().find((object: fabric.Object) => object.name === 'clip');
  };
  ...
  return {
    ...
    delete: () => {
      canvas.getActiveObjects().forEach((object: fabric.Object) => canvas.remove(object));
      canvas.discardActiveObject();
      canvas.renderAll();
    },
  ...
  const init = useCallback(
    ({
      ...
      const initialWorkspace = new fabric.Rect({
        width: initialWidth.current || 1920,
        height: initialHeight.current || 1080,
        name: 'clip',
        fill: 'white',
        selectable: false,
        hasControls: false,
